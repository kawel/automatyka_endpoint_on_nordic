...
{
    COAP_TRC("[COAPobs]: >> coap_observe_server_init.\r\n");
...
{
    COAP_TRC("[COAPobs]: >> observe_server_register\r\n");
...
{
    COAP_TRC("[COAPobs]: >> observe_server_unregister\r\n");
...
{
    COAP_TRC("[COAPobs]: >> observe_client_register\r\n");
...
{
    COAP_TRC("[COAPobs]: >> observe_client_unregister\r\n");
...
{
    COAP_TRC("[COAPobs]: >> observe_client_send_handle\r\n");
    ...
    if (p_request->header.code == COAP_CODE_GET)
    {
        ...
        if (observe_option == 1)
        {
            ...
            if (err_code == NRF_SUCCESS)
            {
                ...
                COAP_TRC("[COAPobs]: OBSERVE=1 in request, client_unregister handle: %i\r\n", handle);
...
{
    COAP_TRC("[COAPobs]: >> observe_client_response_handle\r\n");
        ...
        if (p_item == NULL)
        {
            ...
            if (err_code == NRF_SUCCESS)
            {
                ...
                if (err_code == NRF_SUCCESS)
                {
                    ...
                    COAP_TRC("[COAPobs]: Notification received on handle: %i\r\n", handle);
        ...
        else // p_item set.
        {
            ...
            if (err_code == (NRF_ERROR_NOT_FOUND))
            {
                ...
                if (p_response->header.code == COAP_CODE_205_CONTENT)
                {
                    ...
                    COAP_TRC("[COAPobs]: Subscription response received, client_register handle: %i\r\n", observable_resource_handle);
                }
            }
        }
    }
    else // COAP_OPT_OBSERVE not present
    {
        ...
        {
            (void)internal_coap_observe_client_unregister(handle);
            COAP_TRC("[COAPobs]: OBSERVE not present in notification, client_unregister handle: %i\r\n", handle);
        }
    }
