...
/**@defgroup COAP_ERROR_CODES Module error codes.
 * @{ */
#define COAP_MESSAGE_ERROR_NULL          0x0040
#define COAP_MESSAGE_ERROR_INVALID_CONF  0x0041
#define COAP_MESSAGE_INVALID_CONTENT     0x0042
#define COAP_TRANSMISSION_RESET_BY_PEER  0x0043
#define COAP_TRANSMISSION_TIMEOUT        0x0044
#define COAP_TRANSPORT_SECURITY_MISSING  0x0045
/**@} */
...
/**@brief Handler function for manually handling all incomming requests.
 *
...
typedef struct
{
    ...
    coap_port_t              port;                                /**< Transport layer variable to associate the message with an underlying Transport Layer socket descriptor. \
                                                                       Will be populated by the library during creation of the message. \
                                                                       A value of 0 will auto-select port according to the type of message sent. Requests will go on COAP_CLIENT_PORT, \
                                                                       and response will go on COAP_SERVER_PORT. */
} coap_message_conf_t;
...
/**@brief Register request handler which should handle all incomming requests.
 *
 ...
 *                       given as in this parameter. Rationale: The server is not envisioned to be
